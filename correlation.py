...
class CorrelationReduction:
    ...
    def factor_reduction(self,n_factors=3):
...
def main():
    ...
    correlation.factor_reduction(n_factors=3)
    dZ = correlation.correlated_dZ(n_sim=50)
    print(pd.DataFrame(dZ[:,:,0]))


    plt.figure()
    plt.title('Correlated Random Numbers')
    plt.scatter(dZ[:,1,:],dZ[:,2,:])
    plt.xlabel('$F_{t=1}$')
    plt.ylabel('$F_{t=1.5}$')
    plt.show()
